...
exports.getInfo = function(input, callback, params) {
	...
	fs.stat(input, function(err, stat) {
		...
		// and get ID3 or filename results
			...
			exports.readFile(input, callback, params);
		}
		else {
 ...
 * Looks for ID3 tags.
 * Falls back to filename search if ID3 fails
 *
 * Params used mainly for filename search,
 * in order to define custom format
 */

exports.readFile = function(file, callback, params) {
	fs.readFile(file, function(err, data) {
		...
		if (!tags || (!tags.title && !tags.id)) {
			exports._infoFromPath(file, callback, params);
		}
...
/*
 * Get artist info from path/file
 * Callback: Call after processing
 * Custom: Custom format using parts
 * Object: { search: '<artist>/<album>/<track>_<title>', usedir: (true|false) }
 */

exports._infoFromPath = function(path, callback, custom) {
	...
	// Add custom search to formats
	if (custom) {
		formats.push(custom);
	}
	
	var foundMatch = false; // Flag for match
	...
	// If no match found, allow for formats
	if (!foundMatch) {
		callback(false, {
			found: false,
