...
exports._infoFromPath = function(path, callback) {
	...
	var formats = [
		...
		},
		{
			...
			usedir: true,
			search: /^(.+)\/(.+)\/([0-9]+)\s?(?:-|_)\s?(.+)$/gi,
			result: ['artist','album','track','title']
		},
		{
			// Artist/Track - Title
			usedir: true,
	...
	console.log('[FP] Checking '+fdir);
	for (reg in formats) {
		var obj = formats[reg];
		var result = obj.result;
		var match = obj.search.exec(obj.usedir ? fdir : fname);
		console.log('[FP] '+match);
